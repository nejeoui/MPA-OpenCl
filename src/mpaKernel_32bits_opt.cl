...
static inline uint addc(uint a, uint b, uint cin, uint *cout)
{
#if MPA_MULHI
    uint s = a + b;
    uint c = (s < a) ? 1u : 0u;
...
static inline uint subb(uint a, uint b, uint bin, uint *bout)
{
#if MPA_MULHI
    uint d = a - b;
    uint br = (a < b) ? 1u : 0u;
...
static inline void mac(uint a, uint b, uint c, uint d, uint *hi, uint *lo)
{
#if MPA_MULHI
    uint l = a * b;
    uint h = mul_hi(a, b);
...
static inline int cmpT(const uint *a, const uint *b)
{
    UNROLL
    for (int i = 0; i < T; i++) {
        if (a[i] > b[i]) return  1;
...
static inline void subInPlace(uint *r, const uint *p)
{
    uint borrow = 0;
    UNROLL
...
static inline void addInPlace(uint *r, const uint *p)
{
    uint carry = 0;
    UNROLL
...
static inline void loadT(uint *dst, __global const uint *src, size_t g)
{
    UNROLL
    for (int i = 0; i < T; i++) dst[i] = src[IDX(g, i)];
}

static inline void storeT(__global uint *dst, const uint *src, size_t g)
{
    UNROLL
    for (int i = 0; i < T; i++) dst[IDX(g, i)] = src[i];
}

static inline void op_add(__global const uint *x, __global const uint *y,
...
static inline void op_sub(__global const uint *x, __global const uint *y,
                   __global uint *out, size_t g)
{
    uint borrow = 0;
    UNROLL
...
static inline void op_addmod(__global const uint *x, __global const uint *y,
...
static inline void op_submod(__global const uint *x, __global const uint *y,
                      __global uint *out, size_t g, const uint *p)
{
#if MPA_REGACC
    uint r[T];
...
static inline void op_mul_operand(__global const uint *x, __global const uint *y,
                           __global uint *out, size_t g)
{
#if MPA_REGACC
    uint acc[2 * T];
...
static inline void op_mul_product(__global const uint *x, __global const uint *y,
                           __global uint *out, size_t g)
{
...
static inline void copyN(uint *d, const uint *s)
{
    UNROLL
    for (int i = 0; i < T; i++) d[i] = s[i];
}

static inline void zeroN(uint *a)
{
    UNROLL
    for (int i = 0; i < T; i++) a[i] = 0;
}

static inline void oneN(uint *a)
{
    zeroN(a);
    a[T - 1] = 1;
}

static inline int isZeroN(const uint *a)
{
    uint acc = 0;
    UNROLL
...
static inline int cmpN(const uint *a, const uint *b)
{
    for (int i = 0; i < T; i++) {
        if (a[i] > b[i]) return  1;
        if (a[i] < b[i]) return -1;
...
static inline uint addN(uint *r, const uint *a, const uint *b)
{
    uint carry = 0;
    UNROLL
...
static inline uint subN(uint *r, const uint *a, const uint *b)
{
    uint borrow = 0;
    UNROLL
...
static inline uint shl1N(uint *a)
{
    uint carry = 0;
    UNROLL
    for (int i = T - 1; i >= 0; i--) {
...
static inline void shr1N(uint *a)
{
    uint carry = 0;
    UNROLL
    for (int i = 0; i < T; i++) {
...
static inline uint bitAtN(const uint *a, int bit)
{
    const int w = T - 1 - (bit >> 5);
    return (a[w] >> (bit & 31)) & 1u;
}

static inline int bitLenN(const uint *a)
{
    for (int i = 0; i < T; i++) {
        if (a[i]) {
            int b = 0;
...
static inline void mulLowN(uint *r, const uint *a, const uint *b)
{
    uint acc[T];
    zeroN(acc);
    for (int i = T - 1; i >= 0; i--) {
...
static inline void montMulPriv(uint *out, const uint *xbe, const uint *ybe,
...
static inline void modDoubleN(uint *r, const uint *p)
{
    const uint c = shl1N(r);
    if (c || cmpN(r, p) >= 0) { uint tmp[T]; subN(tmp, r, p); copyN(r, tmp); }
}

static inline void modAddN(uint *r, const uint *a, const uint *p)
{
    uint tmp[T];
...
static inline void divModN(uint *q, uint *r, const uint *a, const uint *b)
{
    zeroN(q);
    zeroN(r);
    if (isZeroN(b)) return;
...
static inline void op_compare(__global const uint *x, __global const uint *y,
                       __global uint *out, size_t g)
{
    int c = 0;
    for (int i = 0; i < T; i++) {
...
static inline void op_modmul(__global const uint *x, __global const uint *y,
...
static inline void op_modmul_r2(__global const uint *x, __global const uint *y,
                         __global uint *out, size_t g, const uint *p,
...
static inline void op_divide(__global const uint *x, __global const uint *y,
                      __global uint *out, size_t g)
{
...
static inline void op_isqrt(__global const uint *x, __global uint *out, size_t g)
{
...
static inline void op_montgomery(__global const uint *x, __global const uint *y,
                          __global uint *out, size_t g,
